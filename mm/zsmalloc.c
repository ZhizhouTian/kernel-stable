...
static void obj_free(struct size_class *class, unsigned long obj)
{
	...
	void *vaddr;
	int class_idx;			// unsed, should deleted
	enum fullness_group fullness;	// unsed, should deleted
...
static int zs_register_migration(struct zs_pool *pool)
{
	pool->inode = anon_inode_mkinode(zsmalloc_mnt->mnt_sb);
				...
				struct size_class *class)
{
	...
	while ((src_zspage = isolate_zspage(class, true))) {
		...
		nr_to_migrate = src_zspage->inuse;
		cc.index = 0;
