...
static __attribute__((optimize("O0"))) enum fullness_group get_fullness_group(struct page *page)
{
...
static __attribute__((optimize("O0"))) struct page *get_next_page(struct page *page)
{
	struct page *next;
 ...
 */
static __attribute__((optimize("O0"))) void *obj_location_to_handle(struct page *page, unsigned long obj_idx)
{
 ...
 */
static __attribute__((optimize("O0"))) void obj_handle_to_location(unsigned long handle, struct page **page,
				unsigned long *obj_idx)
{
...
static __attribute__((optimize("O0"))) unsigned long obj_idx_to_offset(struct page *page,
 ...
 */
static __attribute__((optimize("O0"))) struct page *
alloc_zspage(struct size_class *class, gfp_t flags)
{
	int i, error;
...
unsigned long zs_malloc(struct zs_pool *pool, size_t size)
{
	...
	objidx_to_page_and_offset(class, first_page, freeobj_idx,
			&m_page, &m_offset);
	obj = (unsigned long)obj_location_to_handle(m_page, freeobj_idx);

	link = (struct link_free *)kmap_atomic(m_page) +
					m_offset / sizeof(*link);
	set_freeobj(first_page, ((unsigned long)link->next & OBJ_INDEX_MASK));
...
void zs_free(struct zs_pool *pool, unsigned long obj)
{
	...
	link = (struct link_free *)((unsigned char *)kmap_atomic(f_page)
							+ f_offset);
	freeobj_idx = get_freeobj(first_page);
	link->next = obj_location_to_handle(f_page, freeobj_idx);
	kunmap_atomic(link);
	set_freeobj(first_page, obj & OBJ_INDEX_MASK);
